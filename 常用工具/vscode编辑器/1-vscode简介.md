...
6. VS Code不仅可以通过界面进行相关操作，也提供了命令行模式的操作形式；
...
或者点击vscode中的标题栏中的帮助 -> 文档可跳转到官方文档
...
- 在ubuntu下，由于网路原因，一般通过deb包方式安装vscode，卸载的时候通过`dpkg -r`的方式即可完成卸载，如果想进一步清除使用数据可以清除数据保存的文件夹
